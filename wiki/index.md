# Index

Catalog of every page in the wiki. Updated on every ingest.

Use this as your starting point: scan the relevant section, click into the pages you need.

## Personas

- [Employee](personas/employee.md) — the default; anyone on the company roster who isn't HR / manager / admin / finance.
- [HR](personas/hr.md) — people-ops practitioners; the persona Pulse HR is named for.
- [Manager](personas/manager.md) — has at least one direct report.
- [Admin](personas/admin.md) — workspace owner; sees everything.
- [Finance](personas/finance.md) — margin and reimbursement.

## Features

- [Activities](features/activities.md) — flat task list, lighter than an issue tracker.
- [Announcements](features/announcements.md) — company-wide messages.
- [Calendar](features/calendar.md) — team out-of-office and booking view.
- [Clients](features/clients.md) — external parties commesse are billed against.
- [Dashboard](features/dashboard.md) — the personalized home screen.
- [Developers](features/developers.md) — API keys, webhooks, custom fields, audit log.
- [Documents](features/documents.md) — contracts, NDAs, handbooks; templated and signed.
- [Employees](features/employees.md) — the directory.
- [Feedback](features/feedback.md) — public feedback board (separate site).
- [Growth](features/growth.md) — engagement, performance, employee score.
- [In-App Docs](features/in-app-docs.md) — user-facing how-to.
- [Kudos](features/kudos.md) — peer recognition coins. NEW.
- [Leave](features/leave.md) — vacation, sick, personal, parental — full and half day.
- [Marketplace](features/marketplace.md) — integrations index.
- [Modules](features/modules.md) — admin-only feature toggle per role.
- [Moments](features/moments.md) — birthdays, anniversaries, kudos ticker.
- [Offices](features/offices.md) — physical workspaces, desk and room booking.
- [Onboarding](features/onboarding.md) — auto-workflow for new hires.
- [Org Chart](features/org-chart.md) — visual reporting tree.
- [Projects](features/projects.md) — single-commessa detail view.
- [Pulse](features/pulse.md) — anonymous weekly vibe heatmap. NEW.
- [Recruiting](features/recruiting.md) — kanban hiring pipeline.
- [Reports](features/reports.md) — aggregate numbers, drill-downs, exports.
- [Saturation](features/saturation.md) — utilization heatmap; at-risk projects.
- [Settings](features/settings.md) — workspace and personal preferences.
- [Status Log](features/status-log.md) — daily AI conversation; manager sees only the summary.
- [Time Tracking](features/time-tracking.md) — log hours against a commessa.

## Entities

- [Activity](entities/activity.md) — task; todo / in_progress / review / done / blocked.
- [Allocation](entities/allocation.md) — person assigned to a commessa at a percentage.
- [Candidate](entities/candidate.md) — pre-hire pipeline record.
- [Client](entities/client.md) — external party; groups commesse.
- [Commessa](entities/commessa.md) — billable project code; the pivot entity.
- [Department](entities/department.md) — functional grouping of employees.
- [Document](entities/document.md) — contract, NDA, handbook, receipt.
- [Employee](entities/employee.md) — person on the company roster.
- [Employee Score](entities/employee-score.md) — 0..100 health and engagement number.
- [Job Posting](entities/job-posting.md) — open role; container for candidates.
- [Kudo](entities/kudo.md) — peer recognition coin.
- [Leave Request](entities/leave-request.md) — vacation / sick / personal / parental.
- [Log Session](entities/log-session.md) — one Status Log conversation.
- [Manager Ask](entities/manager-ask.md) — pre-loaded question from a manager.
- [Office](entities/office.md) — physical workspace with bookable rooms.
- [Onboarding Task](entities/onboarding-task.md) — one step of the new-hire workflow.
- [Plan](entities/plan.md) — lightweight scope doc attached to a commessa.
- [Pulse Entry](entities/pulse-entry.md) — anonymous vibe submission.
- [Timesheet Entry](entities/timesheet-entry.md) — one row of logged time.

## Concepts

- [Command Palette](concepts/command-palette.md) — ⌘K finder for navigation.
- [Copilot](concepts/copilot.md) — ⌘J natural-language action overlay.
- [Demo Mode](concepts/demo-mode.md) — frontend-only mockup state.
- [Keyboard-First](concepts/keyboard-first.md) — design stance; shortcut catalog.
- [Labs](concepts/labs.md) — experimental shelf with NEW-badged features.
- [Open Source Positioning](concepts/open-source-positioning.md) — why Pulse HR ships in public.
- [PWA & Offline](concepts/pwa-offline.md) — installable, offline-capable surfaces.
- [Role Themes](concepts/role-themes.md) — seven themes; per-role color and override.

## Journeys

- [Candidate to Employee](journeys/candidate-to-employee.md) — hire transition.
- [Daily Status Log](journeys/daily-status-log.md) — a regular workday end-to-end.
- [Leave Request to Approval](journeys/leave-request-to-approval.md) — submission to calendar update.
- [Onboarding a New Hire](journeys/onboarding-new-hire.md) — offer signed to first-week productive.
- [Quarterly Pulse Cycle](journeys/quarterly-pulse-cycle.md) — continuous signal to structured 1:1.

## Glossary

- [badge](glossary/badge.md) — clock-in / clock-out (Italian usage of an English loanword).
- [commessa](glossary/commessa.md) — billable project code; the pivot entity.
- [ferie](glossary/ferie.md) — annual paid vacation.
- [malattia](glossary/malattia.md) — sick leave.
- [permesso](glossary/permesso.md) — short personal leave; half-day absences.
- [ROL](glossary/rol.md) — banked hour-leave (*Riduzione Orario di Lavoro*).
