...
import {
  Play, Users, DollarSign, FileText, ChevronRight, Download, Eye, Ban, CheckCircle2, Trash2,
  Wallet, Receipt,
} from "lucide-react";
...
import {
  payrollRuns as seed, payslips as payslipsSeed, employeeById,
  type PayrollRun,
} from "@/lib/mock-data";
...
function Payroll() {
  const [runs, setRuns] = useState<PayrollRun[]>(seed);
  ...
  const removeRun = (r: PayrollRun) => {
    ...
    toast("Payroll run deleted", {
      action: { label: "Undo", onClick: () => setRuns(rs => [r, ...rs]) },
    });
  };

  const markCompleted = (r: PayrollRun) => {
    setRuns(rs => rs.map(x => (x.id === r.id ? { ...x, status: "completed" } : x)));
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto fade-in">
      ...
      {loading ? (
        <Card className="p-6 mb-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-16 shimmer rounded" />)}
          </div>
        </Card>
      ) : !upcoming ? (
        <Card className="p-0 mb-4">
          <EmptyState
            ...
            action={<Button size="sm" onClick={() => openAction("run-payroll")}><Play className="h-4 w-4 mr-1.5" />Run payroll</Button>}
          />
        </Card>
      ) : (
        <Card className="p-6 mb-4 bg-gradient-to-br from-primary/5 to-transparent border-primary/20 relative overflow-hidden">
          <div className="absolute -top-20 -right-20 h-60 w-60 rounded-full bg-primary/10 blur-3xl pointer-events-none" aria-hidden />
          <div className="relative">
            ...
            <div className="mt-5 pt-5 border-t flex items-center justify-between">
              <div className="text-sm text-muted-foreground">Withholding, social security, F24 module, pension contributions</div>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" className="press-scale" onClick={() => setSelectedRun(upcoming)}>
                  <Eye className="h-4 w-4 mr-1.5" />Preview payslips
                </Button>
                <Button size="sm" className="press-scale" onClick={() => openAction("run-payroll")}>
      ...
      <Tabs defaultValue="runs">
        <TabsList>
          <TabsTrigger value="runs"><Wallet className="h-3.5 w-3.5 mr-1.5" />Runs</TabsTrigger>
          <TabsTrigger value="salary"><DollarSign className="h-3.5 w-3.5 mr-1.5" />Salary components</TabsTrigger>
          <TabsTrigger value="tax"><Receipt className="h-3.5 w-3.5 mr-1.5" />Tax filings</TabsTrigger>
        </TabsList>
                ...
                <tbody className="stagger-in">
                  {runs.map(p => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5 text-right font-medium tabular-nums">${p.net.toLocaleString()}</td>
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button className="h-7 w-7 rounded-md hover:bg-muted flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/></svg>
                            </button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => setSelectedRun(p)}><Eye className="h-4 w-4 mr-2" />Preview</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => toast.success("Payslips downloaded")}>
                              <Download className="h-4 w-4 mr-2" />Download payslips
                            </DropdownMenuItem>
                            {p.status !== "completed" && (
          ...
          {loading ? <SkeletonCards cards={4} /> : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 stagger-in">
              {[
                ...
                <Card key={c.label} className="p-4 press-scale hover:shadow-md transition-shadow cursor-pointer" onClick={() => toast(c.label, { description: "Breakdown view coming soon" })}>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <span className="h-2 w-2 rounded-full" style={{ backgroundColor: c.color }} />{c.label}
                  </div>
                  <div className="text-2xl font-semibold mt-1 tabular-nums">${c.v.toLocaleString()}</div>
                </Card>
              ))}
            </div>
          )}
            ...
            <div className="divide-y stagger-in">
              {[
                ...
                <div key={t.id} className="px-5 py-3.5 flex items-center gap-4">
                  <div className="h-9 w-9 rounded-md bg-muted flex items-center justify-center"><Receipt className="h-4 w-4 text-muted-foreground" /></div>
                  ...
                  <Button size="sm" variant="outline" className="press-scale" onClick={() => toast.success(`${t.l} filing opened`)}>Open</Button>
                </div>
              ))}
            </div>
          </Card>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => { if (toDelete) removeRun(toDelete); setToDelete(null); }}
  ...
  return (
    <div className="p-5">
      <div className="grid grid-cols-3 gap-3 mb-5">
        ...
        <Stat icon={<DollarSign className="h-4 w-4" />} label="Gross" value={`$${run.gross.toLocaleString()}`} />
        <Stat icon={<DollarSign className="h-4 w-4" />} label="Net"   value={`$${run.net.toLocaleString()}`} />
      </div>
      {slips.length === 0 ? (
        <EmptyState compact icon={<Ban className="h-6 w-6" />} title="No payslips yet" description="Payslips appear once the run is processed." />
      ) : (
            ...
            return (
              <div key={s.id} className="px-4 py-3 flex items-center gap-3 hover:bg-muted/40 transition-colors">
                ...
                <div className="text-right">
                  <div className="text-sm font-semibold tabular-nums">${s.net.toLocaleString()}</div>
                  <div className="text-[11px] text-muted-foreground tabular-nums">gross ${s.gross.toLocaleString()}</div>
                </div>
      ...
      <Button className="w-full mt-5 press-scale" variant="outline" onClick={() => toast.success("Bulk download started")}>
        <Download className="h-4 w-4 mr-1.5" />Download all payslips (PDF)
      </Button>
    </div>
  );
}
