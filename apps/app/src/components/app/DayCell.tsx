...
const STATUS_BG: Record<DayStatus, string> = {
  ...
  holiday:
    "bg-cal-holiday/10 hover:bg-cal-holiday/15 border-cal-holiday/30 [background-image:repeating-linear-gradient(45deg,transparent,transparent_4px,color-mix(in_oklch,var(--color-border)_50%,transparent)_4px,color-mix(in_oklch,var(--color-border)_50%,transparent)_5px)]",
...
export const DayCell = forwardRef<HTMLButtonElement, Props>(function DayCell(
  ...
  const hasTooltip =
    info.entries.length > 0 || !!info.leave || !!info.holiday || info.status === "missing";
    ...
    <div className="text-xs bg-popover text-popover-foreground">
      <div className="px-3 py-2 border-b flex items-center justify-between gap-2">
        <div className="font-semibold text-popover-foreground">
          {format(info.date, "EEE, MMM d")}
        </div>
        {info.hours > 0 && (
          <div className="font-mono tabular-nums text-[11px] text-popover-foreground">
            ...
            return (
              <li key={e.id} className="px-3 py-2 flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] leading-snug mt-0.5 text-popover-foreground">
                    {e.description}
                  </div>
                </div>
              </li>
      ...
      {info.entries.length === 0 && info.hours === 0 && info.status !== "missing" && (
        <div className="px-3 py-2 text-[11px] text-muted-foreground">No entries.</div>
      )}
    </div>
  );
}
