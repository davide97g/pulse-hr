...
export function levelFromXp(xp: number): { level: Level; next?: Level; progressPct: number } {
  ...
  const progressPct =
    span === 0 ? 100 : Math.min(100, Math.max(0, Math.round(((xp - lvl.xpMin) / span) * 100)));
...
export function badgesFor(employeeId: string): Badge[] {
  ...
  const challengesDone = challengesSeed.filter(
    (c) => c.employeeId === employeeId && c.status === "succeeded",
  ).length;
  ...
  const def = [
    ...
    },
    {
      ...
      name: "All-hands",
      emoji: "🫂",
      desc: "Receive kudos from 5 different people.",
      have: distinctGivers,
      need: 5,
    },
    {
    ...
    },
    {
      ...
      name: "Risk taker",
      emoji: "⚡",
      desc: "Succeed on 3 stretch challenges.",
      have: challengesDone,
      need: 3,
    },
  ];
...
export function xpInRange(employeeId: string, from: Date, to: Date): number {
  ...
  const goals =
    goalsSeed.filter(
      (g) => g.employeeId === employeeId && g.status === "hit" && inRange(g.dueAt, from, to),
    ).length * 100;
  const oneOnOnes =
