...
import { Progress } from "@pulse-hr/ui/primitives/progress";
...
function Onboarding() {
  ...
  const updateWorkflow = (id: string, patch: Partial<OnboardingWorkflow>) =>
    onboardingWorkflowsTable.update(id, patch);
  ...
  const createWorkflow = (data: {
    ...
    onboardingWorkflowsTable.add(w);
    setActiveId(w.id);
    toast.success(`${w.type === "onboarding" ? "Onboarding" : "Offboarding"} started`, {
      description: `For ${w.name}`,
    });
  };

  const removeWorkflow = (w: OnboardingWorkflow) => {
    onboardingWorkflowsTable.remove(w.id);
    if (activeId === w.id) setActiveId(workflows.find((x) => x.id !== w.id)?.id ?? "");
    toast("Workflow deleted", {
      action: { label: "Undo", onClick: () => onboardingWorkflowsTable.add(w) },
    });
  };
      ...
      <Tabs value={tab} onValueChange={setTab} className="mb-4">
        <TabsList>
          <TabsTrigger value="active">
            ...
            Active ({workflows.filter((w) => w.type === "onboarding").length})
          </TabsTrigger>
          <TabsTrigger value="offboarding">
            <UserMinus className="h-3.5 w-3.5 mr-1.5" />
            Offboarding ({workflows.filter((w) => w.type === "offboarding").length})
          </TabsTrigger>
        </TabsList>
      ...
      <AlertDialog open={!!toDelete} onOpenChange={(o) => !o && setToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {toDelete && `${toDelete.name}'s ${toDelete.type} workflow will be removed.`}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => {
                if (toDelete) removeWorkflow(toDelete);
                setToDelete(null);
              }}
            >
              Delete
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
      ...
      {active ? (
          ...
          <div className="p-5 border-b flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-muted-foreground">
                {active.role} · Starts {active.startDate}
              </div>
            </div>
            <div className="flex gap-1">
              <Button size="sm" variant="outline" className="press-scale" onClick={onAddTask}>
              ...
              <ul className="space-y-1">
                {active.tasks.map((t) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-muted-foreground hidden md:block w-24 truncate">
                      {t.owner}
                    </div>
                    <button
        ...
        <Card className="p-0">
          <EmptyState compact icon={<Pencil className="h-6 w-6" />} title="Select a workflow" />
        </Card>
      )}
    </div>
  );
...
function NewTaskForm({
  ...
  onCancel: () => void;
  onSave: (d: Omit<OnboardingTask, "id" | "done">) => void;
}) {
    ...
    <>
      <div className="p-5 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Select
            value={category}
            onValueChange={(v) => setCategory(v as OnboardingTask["category"])}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {(["Paperwork", "Access", "People", "Training"] as const).map((c) => (
      ...
      <div className="px-5 py-3 border-t flex justify-end gap-2 sticky bottom-0 bg-card">
        ...
        <Button disabled={!label.trim()} onClick={() => onSave({ label, owner, category, due })}>
          Add task
        </Button>
      </div>
    </>
    ...
    <>
      <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["onboarding", "offboarding"] as const).map((t) => (
              <button
                ...
                onClick={() => setType(t)}
                className={cn(
                  "text-xs py-2.5 rounded-md border capitalize press-scale",
                  type === t
        ...
        <div className="space-y-1.5">
          ...
          <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Emma Wilson" />
        </div>
        <div className="space-y-1.5">
          <Label>Role</Label>
          <Input
        ...
        <div className="space-y-1.5">
          <Label>{type === "onboarding" ? "Start date" : "Last day"}</Label>
          <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
        </div>
        <div className="rounded-md bg-info/5 border border-info/20 p-3 text-xs">
          A default task template for <strong>{type}</strong> will be pre-filled. You can edit tasks
          afterwards.
        </div>
      </div>
      <div className="px-5 py-3 border-t flex justify-end gap-2 sticky bottom-0 bg-card">
        ...
        <Button
          ...
          onClick={() => onSave({ name, role, startDate, type })}
        >
          Create workflow
        </Button>
      </div>
