 ...
 * Handlers should import `notifyUser` and `queueEmail` rather than writing to
 * the tables directly, so the two always stay consistent and per-user
 ...
 * configured (e.g. local dev without a secret key) — callers should degrade
 * gracefully.
 */
export async function listWorkspaceMembers(): Promise<MemberLite[]> {
  ...
  // Clerk caps per-page at 500; we paginate defensively.
  let offset = 0;
  const limit = 200;
  for (;;) {
...
/** Subset of members considered admins (role === "admin" OR email allowlisted). */
 ...
 * (case-insensitive, longest-match-first so "@Andrea Mazza" wins over "@Andrea").
 * Returns the distinct Clerk user ids that match.
 */
export function parseMentions(text: string, members: MemberLite[]): string[] {
  if (!text.includes("@") || members.length === 0) return [];
  // Sort by name length desc so multi-word names get matched before prefixes.
